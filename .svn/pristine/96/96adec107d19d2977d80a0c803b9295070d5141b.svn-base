	...
	class PDO {
		...
		function PDO($string_dsn, $string_username = '', $string_password = '', $array_driver_options = null) {
			...
			if($con['dbtype'] === 'mysql') {
				...
				if(isset($con['port'])) $con['host'] .= ':' . $con['port'];
				$this->__driver = new PDO_mysql($con['host'], $con['dbname'], $string_username, $string_password);
				...
				if(isset($con['port'])) $string_dsn .= " port={$con['port']}";
