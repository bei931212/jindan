...
class QRinput {
	...
	public static function checkModeAn($size, $data) {
		for($i = 0; $i < $size; $i ++) {
			if(self::lookAnTable(ord($data[$i])) == - 1) {return false;}
		}
	...
	public static function checkModeKanji($size, $data) {
		...
		for($i = 0; $i < $size; $i += 2) {
			...
			if($val < 0x8140 || ($val > 0x9ffc && $val < 0xe040) || $val > 0xebbf) {return false;}
		}
	...
	public function identifyMode($pos) {
		...
		if(self::isdigitat($this->dataStr, $pos)) {
			...
			if($pos + 1 < strlen($this->dataStr)) {
				...
				$word = (ord($c) << 8) | ord($d);
				if(($word >= 0x8140 && $word <= 0x9ffc) || ($word >= 0xe040 && $word <= 0xebbf)) {return QR_MODE_KANJI;}
			}
		}
...
class QRrawcode {
	...
	public function __construct(QRinput $input) {
		...
		if(is_null($this->datacode)) {throw new Exception('null imput string');}
