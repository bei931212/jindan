...
class DB extends Api {
	...
	public function __construct($name = 'default') {
		...
		if(empty($cfg)) {
			return self::responseError(-100, "没有找到名为 {$name} 的数据库配置项.");
		}
		...
		if(class_exists('PDO')) {
			if(extension_loaded("pdo_mysql") && in_array('mysql', PDO::getAvailableDrivers())) {
				...
				include IA_ROOT . '/libs/source/pdo/PDO.class.php';
				$dbclass = '_PDO';
			}
		} else {
			include IA_ROOT . '/libs/source/pdo/PDO.class.php';
	...
	public function query($sql, $params = array()) {
		...
		if(! is_object($statement)) {
			$this->debug(false, array(
				'sql' => $sql,
				'error' => array(
					'',
					'-1',
					'当前连接数据库用户没有执行该条语句的权限，请检查mysql权限配置'
				)
			));
			return self::responseError(-101, "{$sql}\n{$this->pdo->errorInfo()}");
			return false;
		}
		...
		if(!$result) {
			if($statement->errorInfo()) {
				return self::responseError(-102, "{$sql}\n{$this->pdo->errorInfo()}");
			}
			return false;
		} else {
	...
	public function fetchcolumn($sql, $params = array(), $column = 0) {
		...
		if(! is_object($statement)) {
			...
			return self::responseError(-103, "{$sql}\n{$this->pdo->errorInfo()}");
			return false;
		}
		$result = $statement->execute($params);
		if(PDO_DEBUG) {
		...
		if(!$result) {
			if($statement->errorInfo()) {
				return self::responseError(-104, $statement->errorInfo());
			}
			return false;
		} else {
	...
	public function fetch($sql, $params = array()) {
		...
		if(! is_object($statement)) {
			...
			return self::responseError(-105, $this->pdo->errorInfo());
			return false;
		}
		...
		if(!$result) {
			if($statement->errorInfo()) {
				return self::responseError(-106, $statement->errorInfo());
			}
			return false;
		} else {
	...
	public function fetchall($sql, $params = array(), $keyfield = '') {
		...
		if(! is_object($statement)) {
			...
			return self::responseError(-107, "{$sql}\n{$this->pdo->errorInfo()}");
			return false;
		}
		$result = $statement->execute($params);
		if(PDO_DEBUG) {
		...
		if(!$result) {
			if($statement->errorInfo()) {
				return self::responseError(-108, "{$sql}\n{$this->pdo->errorInfo()}");
			}
			//exit('SQL Error');
			return false;
		} else {
		...
		if(! empty($indexname)) {
			...
			if(! empty($indexs) && is_array($indexs)) {
				foreach($indexs as $row) {
					if($row['Key_name'] == $indexname) {return true;}
				}
			}
		}
		return false;
