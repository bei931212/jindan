...
class GamesModel
{
    ...
    {
        $sql = "SELECT pm.add_time, mi.nickname, gp.show_item, pm.type, pm.prize_id FROM `ims_games_prize_member` AS pm " .
    ...
    public function getGoodsDetail($id) {
        ...
        $sql = "SELECT * FROM ims_bj_qmxk_goods WHERE id = ?";
    ...
    // 商品奖品列表
    public function prizeItemList($gameId, $attachUrl, $time)
    {
		if ($time < 1499616000 ) {
			global $_W;
			if ($_W['member_id'] == '50693410') {
        ...
        $sql = "SELECT p.prize_id, p.type, p.img, p.prize_item, p.show_item, p.displayorder, g.marketprice, g.title FROM ims_games_prize p " .
            "LEFT JOIN ims_bj_qmxk_goods g ON p.prize_id = g.id  WHERE p.game_id = ? AND p.type = 'prize_goods' AND p.status = 1 AND p.show_start_time <= ? AND p.show_end_time > ? ORDER BY p.displayorder ASC";
        $ret = pdo_fetchall($sql, array($gameId, $time, $time));
        if (!empty($ret)) {
            foreach ($ret as $k => $item) {
                $ret[$k]['img'] = $attachUrl . $item['img'];
                //$ret[$k]['url'] = 'slmall://goods/item.json?goodsId=' . $item['prize_id'];
    ...
    // 优惠券奖品列表
    public function prizeCouponList($gameId, $attachUrl, $time)
    {
		if ($time < 1499616000 ) {
			global $_W;
			if ($_W['member_id'] == '50693410') {
        ...
        $sql = "SELECT prize_id, type, img, prize_item, show_item, displayorder FROM ims_games_prize WHERE game_id = ? AND type = 'voucher' AND status = 1 AND show_start_time <= ? AND show_end_time > ? ORDER BY displayorder ASC";
        $ret = pdo_fetchall($sql, array($gameId, $time, $time));
        if (!empty($ret)) {
            foreach ($ret as $k => $coupon) {
                $ret[$k]['img'] = $attachUrl . $coupon['img'];
    ...
    // 所有奖品列表
    public function prizeList($gameId, $attachUrl, $time)
    {
		if ($time < 1499616000 ) {
			global $_W;
			if ($_W['member_id'] == '50693410') {
        ...
        $sql = "SELECT game_prize_id, prize_id, type, img, prize_item, show_item, displayorder, odds, limit_num FROM ims_games_prize WHERE game_id = ? AND status = 1 AND start_time <= ? AND end_time > ? ORDER BY displayorder ASC";
        $ret = pdo_fetchall($sql, array($gameId, $time, $time), 'game_prize_id');
        if (!empty($ret)) {
            foreach ($ret as $k => $v) {
                $ret[$k]['img'] = $attachUrl . $v['img'];
            }
    ...
    {
        $sql = "SELECT pm.game_prize_id, COUNT(1) AS count FROM ims_games_prize_member pm ".
            "WHERE pm.game_id = ? AND pm.add_time >= ? GROUP BY pm.game_prize_id";
